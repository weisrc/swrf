...
export type Attributes<T extends HTMLTag> = {
    [key in keyof AttributeMap<T>]-?: (value: Gettable<AttributeMap<T>[key]>) => Gettable<AttributeMap<T>>;
};
export type Style = {
...
export type AttributeMap<T extends HTMLTag> = {
    [key in keyof HTMLMap[T]]?: key extends `on${EventName}` ? HTMLMap[T][key] : key extends "style" ? Gettable<Style> | Gettable<string> : key extends "classList" ? ClassList : Gettable<HTMLMap[T][key]>;
} & MountAttributes;
